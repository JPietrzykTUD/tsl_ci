...
/*
 * \file /home/jpietrzyk/tmp/TSL_ACTIONS/tsl_ci/generated_tsl/src/test/mask_unit_test.cpp
 * \date 2024-02-28
...
TEST_CASE("Testing primitive 'imask_population_count' for avx2", "[imask_population_count],[avx2]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_imask_population_count_default<simd<double, avx2>>());

        CHECK(test_imask_population_count_default<simd<float, avx2>>());

        CHECK(test_imask_population_count_default<simd<int16_t, avx2>>());

        CHECK(test_imask_population_count_default<simd<int32_t, avx2>>());

        CHECK(test_imask_population_count_default<simd<int64_t, avx2>>());

        CHECK(test_imask_population_count_default<simd<int8_t, avx2>>());

        CHECK(test_imask_population_count_default<simd<uint16_t, avx2>>());

        CHECK(test_imask_population_count_default<simd<uint32_t, avx2>>());

        CHECK(test_imask_population_count_default<simd<uint64_t, avx2>>());

        CHECK(test_imask_population_count_default<simd<uint8_t, avx2>>());
...
TEST_CASE("Testing primitive 'to_integral' for avx2", "[to_integral],[avx2]") {
    ...
    SECTION("mask_match_alternate") {
        ...
        WARN("Primitive to_integral<simd<int16_t, avx2>> not implemented.");
        ...
        WARN("Primitive to_integral<simd<int8_t, avx2>> not implemented.");
        WARN("Primitive to_integral<simd<uint16_t, avx2>> not implemented.");
        ...
        WARN("Primitive to_integral<simd<uint8_t, avx2>> not implemented.");
    }
    SECTION("mask_match_zero_to_vec_count") {
