...
/*
 * \file /home/jpietrzyk/tmp/TSL_ACTIONS/tsl_ci/sse_only/src/test/misc_unit_test.cpp
 * \date 2024-02-29
...
TEST_CASE("Testing primitive 'conflict' for sse", "[conflict],[sse]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive conflict<simd<double, sse>> not implemented.");
        WARN("Primitive conflict<simd<float, sse>> not implemented.");
        CHECK(test_conflict_default<simd<int16_t, sse>>());

        CHECK(test_conflict_default<simd<int32_t, sse>>());

        CHECK(test_conflict_default<simd<int64_t, sse>>());

        CHECK(test_conflict_default<simd<int8_t, sse>>());

        CHECK(test_conflict_default<simd<uint16_t, sse>>());

        CHECK(test_conflict_default<simd<uint32_t, sse>>());

        CHECK(test_conflict_default<simd<uint64_t, sse>>());

        CHECK(test_conflict_default<simd<uint8_t, sse>>());
...
TEST_CASE("Testing primitive 'imask_conflict_free' for sse", "[imask_conflict_free],[sse]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive imask_conflict_free<simd<double, sse>> not implemented.");
        WARN("Primitive imask_conflict_free<simd<float, sse>> not implemented.");
        CHECK(test_imask_conflict_free_default<simd<int16_t, sse>>());

        CHECK(test_imask_conflict_free_default<simd<int32_t, sse>>());

        CHECK(test_imask_conflict_free_default<simd<int64_t, sse>>());

        CHECK(test_imask_conflict_free_default<simd<int8_t, sse>>());

        CHECK(test_imask_conflict_free_default<simd<uint16_t, sse>>());

        CHECK(test_imask_conflict_free_default<simd<uint32_t, sse>>());

        CHECK(test_imask_conflict_free_default<simd<uint64_t, sse>>());

        CHECK(test_imask_conflict_free_default<simd<uint8_t, sse>>());
...
TEST_CASE("Testing primitive 'imaskz_mov' for sse", "[imaskz_mov],[sse]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_imaskz_mov_default<simd<double, sse>>());

        CHECK(test_imaskz_mov_default<simd<float, sse>>());

        CHECK(test_imaskz_mov_default<simd<int16_t, sse>>());

        CHECK(test_imaskz_mov_default<simd<int32_t, sse>>());

        CHECK(test_imaskz_mov_default<simd<int64_t, sse>>());

        CHECK(test_imaskz_mov_default<simd<int8_t, sse>>());

        CHECK(test_imaskz_mov_default<simd<uint16_t, sse>>());

        CHECK(test_imaskz_mov_default<simd<uint32_t, sse>>());

        CHECK(test_imaskz_mov_default<simd<uint64_t, sse>>());

        CHECK(test_imaskz_mov_default<simd<uint8_t, sse>>());
...
TEST_CASE("Testing primitive 'mask_mov' for sse", "[mask_mov],[sse]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive mask_mov not implemented for sse");
    }
}
...
TEST_CASE("Testing primitive 'blend' for sse", "[blend],[sse]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive blend<simd<double, sse>> not implemented.");
        WARN("Primitive blend<simd<float, sse>> not implemented.");
        CHECK(test_blend_default<simd<int16_t, sse>>());

        CHECK(test_blend_default<simd<int32_t, sse>>());

        CHECK(test_blend_default<simd<int64_t, sse>>());

        CHECK(test_blend_default<simd<int8_t, sse>>());

        CHECK(test_blend_default<simd<uint16_t, sse>>());

        CHECK(test_blend_default<simd<uint32_t, sse>>());

        CHECK(test_blend_default<simd<uint64_t, sse>>());

        CHECK(test_blend_default<simd<uint8_t, sse>>());
