...
/*
 * \file /home/jpietrzyk/tmp/TSL_ACTIONS/tsl_ci/sse_only/src/test/binary_unit_test.cpp
 * \date 2024-02-29
...
TEST_CASE("Testing primitive 'lzc' for sse", "[lzc],[sse]") {
    using namespace tsl;
    SECTION("test_msb") {
        WARN("Primitive lzc<simd<double, sse>> not implemented.");
        WARN("Primitive lzc<simd<float, sse>> not implemented.");
        CHECK(test_lzc_test_msb<simd<int16_t, sse>>());

        CHECK(test_lzc_test_msb<simd<int32_t, sse>>());

        CHECK(test_lzc_test_msb<simd<int64_t, sse>>());

        CHECK(test_lzc_test_msb<simd<int8_t, sse>>());

        CHECK(test_lzc_test_msb<simd<uint16_t, sse>>());

        CHECK(test_lzc_test_msb<simd<uint32_t, sse>>());

        CHECK(test_lzc_test_msb<simd<uint64_t, sse>>());

        CHECK(test_lzc_test_msb<simd<uint8_t, sse>>());

    }
    SECTION("test_one") {
        WARN("Primitive lzc<simd<double, sse>> not implemented.");
        WARN("Primitive lzc<simd<float, sse>> not implemented.");
        CHECK(test_lzc_test_one<simd<int16_t, sse>>());

        CHECK(test_lzc_test_one<simd<int32_t, sse>>());

        CHECK(test_lzc_test_one<simd<int64_t, sse>>());

        CHECK(test_lzc_test_one<simd<int8_t, sse>>());

        CHECK(test_lzc_test_one<simd<uint16_t, sse>>());

        CHECK(test_lzc_test_one<simd<uint32_t, sse>>());

        CHECK(test_lzc_test_one<simd<uint64_t, sse>>());

        CHECK(test_lzc_test_one<simd<uint8_t, sse>>());

    }
    SECTION("test_zero") {
        WARN("Primitive lzc<simd<double, sse>> not implemented.");
        WARN("Primitive lzc<simd<float, sse>> not implemented.");
        CHECK(test_lzc_test_zero<simd<int16_t, sse>>());

        CHECK(test_lzc_test_zero<simd<int32_t, sse>>());

        CHECK(test_lzc_test_zero<simd<int64_t, sse>>());

        CHECK(test_lzc_test_zero<simd<int8_t, sse>>());

        CHECK(test_lzc_test_zero<simd<uint16_t, sse>>());

        CHECK(test_lzc_test_zero<simd<uint32_t, sse>>());

        CHECK(test_lzc_test_zero<simd<uint64_t, sse>>());

        CHECK(test_lzc_test_zero<simd<uint8_t, sse>>());
...
TEST_CASE("Testing primitive 'shift_left_vector' for sse", "[shift_left_vector],[sse]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive shift_left_vector not implemented for sse");
    }
}
...
TEST_CASE("Testing primitive 'shift_right' for sse", "[shift_right],[sse]") {
    ...
    SECTION("shift_right_arithmetic") {
        ...
        WARN("Primitive shift_right<simd<int64_t, sse>> not implemented.");
...
TEST_CASE("Testing primitive 'shift_right_vector' for sse", "[shift_right_vector],[sse]") {
    using namespace tsl;
    SECTION("shift_right_arithmetic") {
        WARN("Primitive shift_right_vector not implemented for sse");
    }
    SECTION("shift_right_ignore_sign") {
        WARN("Primitive shift_right_vector not implemented for sse");
    }
