...
namespace tsl {
  namespace runtime {
    class cpu {
      public:
        using max_width_extension_t = sse;
        using min_width_extension_t = sse;
      ...
      public:
        ...
        template<typename BaseT, int VectorLength, template<typename...> class Fun, typename... Args>
          decltype(auto) submit(Args... args) {
            ...
            else if constexpr(sizeof(BaseT)*CHAR_BIT*VectorLength == 128) {
        ...
        template<TSLArithmetic BaseT>
          static constexpr std::array<uint32_t, 2> available_parallelism() {
          	return { 1, 128 / (sizeof(BaseT)*CHAR_BIT) };
          }
