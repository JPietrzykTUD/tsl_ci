...
/*
 * \file /home/jpietrzyk/tmp/TSL_ACTIONS/tsl_ci/sse_only/src/test/ls_unit_test.cpp
 * \date 2024-02-29
...
TEST_CASE("Testing primitive 'compress_store' for sse", "[compress_store],[sse]") {
    using namespace tsl;
    SECTION("no_mask") {
        CHECK(test_compress_store_no_mask<simd<double, sse>>());

        CHECK(test_compress_store_no_mask<simd<float, sse>>());

        CHECK(test_compress_store_no_mask<simd<int16_t, sse>>());

        CHECK(test_compress_store_no_mask<simd<int32_t, sse>>());

        CHECK(test_compress_store_no_mask<simd<int64_t, sse>>());

        CHECK(test_compress_store_no_mask<simd<int8_t, sse>>());

        CHECK(test_compress_store_no_mask<simd<uint16_t, sse>>());

        CHECK(test_compress_store_no_mask<simd<uint32_t, sse>>());

        CHECK(test_compress_store_no_mask<simd<uint64_t, sse>>());

        CHECK(test_compress_store_no_mask<simd<uint8_t, sse>>());

    }
    SECTION("full_mask") {
        CHECK(test_compress_store_full_mask<simd<double, sse>>());

        CHECK(test_compress_store_full_mask<simd<float, sse>>());

        CHECK(test_compress_store_full_mask<simd<int16_t, sse>>());

        CHECK(test_compress_store_full_mask<simd<int32_t, sse>>());

        CHECK(test_compress_store_full_mask<simd<int64_t, sse>>());

        CHECK(test_compress_store_full_mask<simd<int8_t, sse>>());

        CHECK(test_compress_store_full_mask<simd<uint16_t, sse>>());

        CHECK(test_compress_store_full_mask<simd<uint32_t, sse>>());

        CHECK(test_compress_store_full_mask<simd<uint64_t, sse>>());

        CHECK(test_compress_store_full_mask<simd<uint8_t, sse>>());
...
TEST_CASE("Testing primitive 'set_zero' for sse", "[set_zero],[sse]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_set_zero_default<simd<double, sse>>());

        CHECK(test_set_zero_default<simd<float, sse>>());

        CHECK(test_set_zero_default<simd<int16_t, sse>>());

        CHECK(test_set_zero_default<simd<int32_t, sse>>());

        CHECK(test_set_zero_default<simd<int64_t, sse>>());

        CHECK(test_set_zero_default<simd<int8_t, sse>>());

        CHECK(test_set_zero_default<simd<uint16_t, sse>>());

        CHECK(test_set_zero_default<simd<uint32_t, sse>>());

        CHECK(test_set_zero_default<simd<uint64_t, sse>>());

        CHECK(test_set_zero_default<simd<uint8_t, sse>>());
...
TEST_CASE("Testing primitive 'masked_set1' for sse", "[masked_set1],[sse]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive masked_set1 not implemented for sse");
    }
}
...
TEST_CASE("Testing primitive 'sequence' for sse", "[sequence],[sse]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_sequence_default<simd<double, sse>>());

        CHECK(test_sequence_default<simd<float, sse>>());

        CHECK(test_sequence_default<simd<int16_t, sse>>());

        CHECK(test_sequence_default<simd<int32_t, sse>>());

        CHECK(test_sequence_default<simd<int64_t, sse>>());

        CHECK(test_sequence_default<simd<int8_t, sse>>());

        CHECK(test_sequence_default<simd<uint16_t, sse>>());

        CHECK(test_sequence_default<simd<uint32_t, sse>>());

        CHECK(test_sequence_default<simd<uint64_t, sse>>());

        CHECK(test_sequence_default<simd<uint8_t, sse>>());
...
TEST_CASE("Testing primitive 'to_array' for sse", "[to_array],[sse]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_to_array_default<simd<double, sse>>());

        CHECK(test_to_array_default<simd<float, sse>>());

        CHECK(test_to_array_default<simd<int16_t, sse>>());

        CHECK(test_to_array_default<simd<int32_t, sse>>());

        CHECK(test_to_array_default<simd<int64_t, sse>>());

        CHECK(test_to_array_default<simd<int8_t, sse>>());

        CHECK(test_to_array_default<simd<uint16_t, sse>>());

        CHECK(test_to_array_default<simd<uint32_t, sse>>());

        CHECK(test_to_array_default<simd<uint64_t, sse>>());

        CHECK(test_to_array_default<simd<uint8_t, sse>>());
...
TEST_CASE("Testing primitive 'custom_sequence' for sse", "[custom_sequence],[sse]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_custom_sequence_default<simd<double, sse>>());

        CHECK(test_custom_sequence_default<simd<float, sse>>());

        CHECK(test_custom_sequence_default<simd<int16_t, sse>>());

        CHECK(test_custom_sequence_default<simd<int32_t, sse>>());

        CHECK(test_custom_sequence_default<simd<int64_t, sse>>());

        CHECK(test_custom_sequence_default<simd<int8_t, sse>>());

        CHECK(test_custom_sequence_default<simd<uint16_t, sse>>());

        CHECK(test_custom_sequence_default<simd<uint32_t, sse>>());

        CHECK(test_custom_sequence_default<simd<uint64_t, sse>>());

        CHECK(test_custom_sequence_default<simd<uint8_t, sse>>());
