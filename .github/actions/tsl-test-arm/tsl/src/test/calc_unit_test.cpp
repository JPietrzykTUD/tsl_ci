...
/*
 * \file /github/workspace/ci/generation/neon/src/test/calc_unit_test.cpp
 * \date 2024-03-08
...
TEST_CASE("Testing primitive 'mod_constant' for neon", "[mod_constant],[neon]") {
    using namespace tsl;
    SECTION("with_check_barrett_reduction_constraint") {
        WARN("Primitive mod_constant not implemented for neon");
    }
    SECTION("with_modulo_one") {
        WARN("Primitive mod_constant not implemented for neon");
    }
...
TEST_CASE("Testing primitive 'mod_safe' for neon", "[mod_safe],[neon]") {
    using namespace tsl;
    SECTION("with_modulo_one") {
        WARN("Primitive mod_safe not implemented for neon");
    }
    SECTION("with_modulo_rand") {
        WARN("Primitive mod_safe not implemented for neon");
    }
...
TEST_CASE("Testing primitive 'mul' for neon", "[mul],[neon]") {
    using namespace tsl;
    SECTION("multiply_neg_and_neg") {
        CHECK(test_mul_multiply_neg_and_neg<simd<double, neon>>());

        CHECK(test_mul_multiply_neg_and_neg<simd<float, neon>>());

        CHECK(test_mul_multiply_neg_and_neg<simd<int16_t, neon>>());

        CHECK(test_mul_multiply_neg_and_neg<simd<int32_t, neon>>());

        CHECK(test_mul_multiply_neg_and_neg<simd<int64_t, neon>>());

        CHECK(test_mul_multiply_neg_and_neg<simd<int8_t, neon>>());

        CHECK(test_mul_multiply_neg_and_neg<simd<uint16_t, neon>>());

        CHECK(test_mul_multiply_neg_and_neg<simd<uint32_t, neon>>());

        CHECK(test_mul_multiply_neg_and_neg<simd<uint64_t, neon>>());

        CHECK(test_mul_multiply_neg_and_neg<simd<uint8_t, neon>>());

    }
    SECTION("multiply_neg_and_pos") {
        CHECK(test_mul_multiply_neg_and_pos<simd<double, neon>>());

        CHECK(test_mul_multiply_neg_and_pos<simd<float, neon>>());

        CHECK(test_mul_multiply_neg_and_pos<simd<int16_t, neon>>());

        CHECK(test_mul_multiply_neg_and_pos<simd<int32_t, neon>>());

        CHECK(test_mul_multiply_neg_and_pos<simd<int64_t, neon>>());

        CHECK(test_mul_multiply_neg_and_pos<simd<int8_t, neon>>());

        CHECK(test_mul_multiply_neg_and_pos<simd<uint16_t, neon>>());

        CHECK(test_mul_multiply_neg_and_pos<simd<uint32_t, neon>>());

        CHECK(test_mul_multiply_neg_and_pos<simd<uint64_t, neon>>());

        CHECK(test_mul_multiply_neg_and_pos<simd<uint8_t, neon>>());

    }
    SECTION("default") {
        CHECK(test_mul_default<simd<double, neon>>());

        CHECK(test_mul_default<simd<float, neon>>());

        CHECK(test_mul_default<simd<int16_t, neon>>());

        CHECK(test_mul_default<simd<int32_t, neon>>());

        CHECK(test_mul_default<simd<int64_t, neon>>());

        CHECK(test_mul_default<simd<int8_t, neon>>());

        CHECK(test_mul_default<simd<uint16_t, neon>>());

        CHECK(test_mul_default<simd<uint32_t, neon>>());

        CHECK(test_mul_default<simd<uint64_t, neon>>());

        CHECK(test_mul_default<simd<uint8_t, neon>>());

    }
    SECTION("multiply_by_zero") {
        CHECK(test_mul_multiply_by_zero<simd<double, neon>>());

        CHECK(test_mul_multiply_by_zero<simd<float, neon>>());

        CHECK(test_mul_multiply_by_zero<simd<int16_t, neon>>());

        CHECK(test_mul_multiply_by_zero<simd<int32_t, neon>>());

        CHECK(test_mul_multiply_by_zero<simd<int64_t, neon>>());

        CHECK(test_mul_multiply_by_zero<simd<int8_t, neon>>());

        CHECK(test_mul_multiply_by_zero<simd<uint16_t, neon>>());

        CHECK(test_mul_multiply_by_zero<simd<uint32_t, neon>>());

        CHECK(test_mul_multiply_by_zero<simd<uint64_t, neon>>());

        CHECK(test_mul_multiply_by_zero<simd<uint8_t, neon>>());
...
TEST_CASE("Testing primitive 'add' for neon", "[add],[neon]") {
    using namespace tsl;
    SECTION("running_sum_w_epsilon") {
        CHECK(test_add_running_sum_w_epsilon<simd<double, neon>>());

        CHECK(test_add_running_sum_w_epsilon<simd<float, neon>>());

        CHECK(test_add_running_sum_w_epsilon<simd<int16_t, neon>>());

        CHECK(test_add_running_sum_w_epsilon<simd<int32_t, neon>>());

        CHECK(test_add_running_sum_w_epsilon<simd<int64_t, neon>>());

        CHECK(test_add_running_sum_w_epsilon<simd<int8_t, neon>>());

        CHECK(test_add_running_sum_w_epsilon<simd<uint16_t, neon>>());

        CHECK(test_add_running_sum_w_epsilon<simd<uint32_t, neon>>());

        CHECK(test_add_running_sum_w_epsilon<simd<uint64_t, neon>>());

        CHECK(test_add_running_sum_w_epsilon<simd<uint8_t, neon>>());

    }
    SECTION("zero_cornercase") {
        CHECK(test_add_zero_cornercase<simd<double, neon>>());

        CHECK(test_add_zero_cornercase<simd<float, neon>>());

        CHECK(test_add_zero_cornercase<simd<int16_t, neon>>());

        CHECK(test_add_zero_cornercase<simd<int32_t, neon>>());

        CHECK(test_add_zero_cornercase<simd<int64_t, neon>>());

        CHECK(test_add_zero_cornercase<simd<int8_t, neon>>());

        CHECK(test_add_zero_cornercase<simd<uint16_t, neon>>());

        CHECK(test_add_zero_cornercase<simd<uint32_t, neon>>());

        CHECK(test_add_zero_cornercase<simd<uint64_t, neon>>());

        CHECK(test_add_zero_cornercase<simd<uint8_t, neon>>());
...
TEST_CASE("Testing primitive 'div' for neon", "[div],[neon]") {
    using namespace tsl;
    SECTION("vec_with_itself") {
        CHECK(test_div_vec_with_itself<simd<double, neon>>());

        CHECK(test_div_vec_with_itself<simd<float, neon>>());

        CHECK(test_div_vec_with_itself<simd<int16_t, neon>>());

        CHECK(test_div_vec_with_itself<simd<int32_t, neon>>());

        CHECK(test_div_vec_with_itself<simd<int64_t, neon>>());

        CHECK(test_div_vec_with_itself<simd<int8_t, neon>>());

        CHECK(test_div_vec_with_itself<simd<uint16_t, neon>>());

        CHECK(test_div_vec_with_itself<simd<uint32_t, neon>>());

        CHECK(test_div_vec_with_itself<simd<uint64_t, neon>>());

        CHECK(test_div_vec_with_itself<simd<uint8_t, neon>>());

    }
    SECTION("vec_with_one") {
        CHECK(test_div_vec_with_one<simd<double, neon>>());

        CHECK(test_div_vec_with_one<simd<float, neon>>());

        CHECK(test_div_vec_with_one<simd<int16_t, neon>>());

        CHECK(test_div_vec_with_one<simd<int32_t, neon>>());

        CHECK(test_div_vec_with_one<simd<int64_t, neon>>());

        CHECK(test_div_vec_with_one<simd<int8_t, neon>>());

        CHECK(test_div_vec_with_one<simd<uint16_t, neon>>());

        CHECK(test_div_vec_with_one<simd<uint32_t, neon>>());

        CHECK(test_div_vec_with_one<simd<uint64_t, neon>>());

        CHECK(test_div_vec_with_one<simd<uint8_t, neon>>());
...
TEST_CASE("Testing primitive 'hmax' for neon", "[hmax],[neon]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive hmax not implemented for neon");
    }
}
...
TEST_CASE("Testing primitive 'hmin' for neon", "[hmin],[neon]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive hmin not implemented for neon");
    }
}
...
TEST_CASE("Testing primitive 'mask_add' for neon", "[mask_add],[neon]") {
    using namespace tsl;
    SECTION("all_one_mask") {
        WARN("Primitive mask_add not implemented for neon");
    }
    SECTION("zero_mask") {
        WARN("Primitive mask_add not implemented for neon");
    }
...
TEST_CASE("Testing primitive 'max' for neon", "[max],[neon]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive max not implemented for neon");
    }
}
...
TEST_CASE("Testing primitive 'min' for neon", "[min],[neon]") {
    using namespace tsl;
    SECTION("min_general_case") {
        CHECK(test_min_min_general_case<simd<double, neon>>());

        CHECK(test_min_min_general_case<simd<float, neon>>());

        WARN("Primitive min<simd<int16_t, neon>> not implemented.");
        CHECK(test_min_min_general_case<simd<int32_t, neon>>());

        WARN("Primitive min<simd<int64_t, neon>> not implemented.");
        CHECK(test_min_min_general_case<simd<int8_t, neon>>());

        CHECK(test_min_min_general_case<simd<uint16_t, neon>>());

        CHECK(test_min_min_general_case<simd<uint32_t, neon>>());

        WARN("Primitive min<simd<uint64_t, neon>> not implemented.");
        CHECK(test_min_min_general_case<simd<uint8_t, neon>>());

    }
    SECTION("min_zero_case") {
        CHECK(test_min_min_zero_case<simd<double, neon>>());

        CHECK(test_min_min_zero_case<simd<float, neon>>());

        WARN("Primitive min<simd<int16_t, neon>> not implemented.");
        CHECK(test_min_min_zero_case<simd<int32_t, neon>>());

        WARN("Primitive min<simd<int64_t, neon>> not implemented.");
        CHECK(test_min_min_zero_case<simd<int8_t, neon>>());

        CHECK(test_min_min_zero_case<simd<uint16_t, neon>>());

        CHECK(test_min_min_zero_case<simd<uint32_t, neon>>());

        WARN("Primitive min<simd<uint64_t, neon>> not implemented.");
        CHECK(test_min_min_zero_case<simd<uint8_t, neon>>());
...
TEST_CASE("Testing primitive 'sub' for neon", "[sub],[neon]") {
    using namespace tsl;
    SECTION("running_sum_w_epsilon") {
        CHECK(test_sub_running_sum_w_epsilon<simd<double, neon>>());

        CHECK(test_sub_running_sum_w_epsilon<simd<float, neon>>());

        CHECK(test_sub_running_sum_w_epsilon<simd<int16_t, neon>>());

        CHECK(test_sub_running_sum_w_epsilon<simd<int32_t, neon>>());

        CHECK(test_sub_running_sum_w_epsilon<simd<int64_t, neon>>());

        CHECK(test_sub_running_sum_w_epsilon<simd<int8_t, neon>>());

        CHECK(test_sub_running_sum_w_epsilon<simd<uint16_t, neon>>());

        CHECK(test_sub_running_sum_w_epsilon<simd<uint32_t, neon>>());

        CHECK(test_sub_running_sum_w_epsilon<simd<uint64_t, neon>>());

        CHECK(test_sub_running_sum_w_epsilon<simd<uint8_t, neon>>());

    }
    SECTION("zero_cornercase") {
        CHECK(test_sub_zero_cornercase<simd<double, neon>>());

        CHECK(test_sub_zero_cornercase<simd<float, neon>>());

        CHECK(test_sub_zero_cornercase<simd<int16_t, neon>>());

        CHECK(test_sub_zero_cornercase<simd<int32_t, neon>>());

        CHECK(test_sub_zero_cornercase<simd<int64_t, neon>>());

        CHECK(test_sub_zero_cornercase<simd<int8_t, neon>>());

        CHECK(test_sub_zero_cornercase<simd<uint16_t, neon>>());

        CHECK(test_sub_zero_cornercase<simd<uint32_t, neon>>());

        CHECK(test_sub_zero_cornercase<simd<uint64_t, neon>>());

        CHECK(test_sub_zero_cornercase<simd<uint8_t, neon>>());
