...
/*
 * \file /github/workspace/ci/generation/neon/src/test/convert_unit_test.cpp
 * \date 2024-03-08
...
TEST_CASE("Testing primitive 'cast' for neon", "[cast],[neon]") {
    using namespace tsl;
    SECTION("default_int_float_conversion") {
        CHECK(test_cast_default_int_float_conversion<simd<double, neon>, simd<uint64_t, neon>>());
        CHECK(test_cast_default_int_float_conversion<simd<double, neon>, simd<int64_t, neon>>());

        CHECK(test_cast_default_int_float_conversion<simd<float, neon>, simd<uint32_t, neon>>());
        CHECK(test_cast_default_int_float_conversion<simd<float, neon>, simd<int32_t, neon>>());

        WARN("Primitive cast<simd<int16_t, neon>> not implemented.");
        CHECK(test_cast_default_int_float_conversion<simd<int32_t, neon>, simd<float, neon>>());

        CHECK(test_cast_default_int_float_conversion<simd<int64_t, neon>, simd<double, neon>>());

        WARN("Primitive cast<simd<int8_t, neon>> not implemented.");
        WARN("Primitive cast<simd<uint16_t, neon>> not implemented.");
        CHECK(test_cast_default_int_float_conversion<simd<uint32_t, neon>, simd<float, neon>>());

        CHECK(test_cast_default_int_float_conversion<simd<uint64_t, neon>, simd<double, neon>>());

        WARN("Primitive cast<simd<uint8_t, neon>> not implemented.");
    }
}
