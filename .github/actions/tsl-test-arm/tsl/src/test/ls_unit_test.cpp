...
/*
 * \file /github/workspace/ci/generation/neon/src/test/ls_unit_test.cpp
 * \date 2024-03-08
...
TEST_CASE("Testing primitive 'store' for neon", "[store],[neon]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_store_default<simd<double, neon>>());

        CHECK(test_store_default<simd<float, neon>>());

        CHECK(test_store_default<simd<int16_t, neon>>());

        CHECK(test_store_default<simd<int32_t, neon>>());

        CHECK(test_store_default<simd<int64_t, neon>>());

        CHECK(test_store_default<simd<int8_t, neon>>());

        CHECK(test_store_default<simd<uint16_t, neon>>());

        CHECK(test_store_default<simd<uint32_t, neon>>());

        CHECK(test_store_default<simd<uint64_t, neon>>());

        CHECK(test_store_default<simd<uint8_t, neon>>());
...
TEST_CASE("Testing primitive 'storeu' for neon", "[storeu],[neon]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_storeu_default<simd<double, neon>>());

        CHECK(test_storeu_default<simd<float, neon>>());

        CHECK(test_storeu_default<simd<int16_t, neon>>());

        CHECK(test_storeu_default<simd<int32_t, neon>>());

        CHECK(test_storeu_default<simd<int64_t, neon>>());

        CHECK(test_storeu_default<simd<int8_t, neon>>());

        CHECK(test_storeu_default<simd<uint16_t, neon>>());

        CHECK(test_storeu_default<simd<uint32_t, neon>>());

        CHECK(test_storeu_default<simd<uint64_t, neon>>());

        CHECK(test_storeu_default<simd<uint8_t, neon>>());
...
TEST_CASE("Testing primitive 'compress_store' for neon", "[compress_store],[neon]") {
    using namespace tsl;
    SECTION("no_mask") {
        WARN("Primitive compress_store not implemented for neon");
    }
    SECTION("full_mask") {
        WARN("Primitive compress_store not implemented for neon");
    }
...
TEST_CASE("Testing primitive 'load' for neon", "[load],[neon]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_load_default<simd<double, neon>>());

        CHECK(test_load_default<simd<float, neon>>());

        CHECK(test_load_default<simd<int16_t, neon>>());

        CHECK(test_load_default<simd<int32_t, neon>>());

        CHECK(test_load_default<simd<int64_t, neon>>());

        CHECK(test_load_default<simd<int8_t, neon>>());

        CHECK(test_load_default<simd<uint16_t, neon>>());

        CHECK(test_load_default<simd<uint32_t, neon>>());

        CHECK(test_load_default<simd<uint64_t, neon>>());

        CHECK(test_load_default<simd<uint8_t, neon>>());
...
TEST_CASE("Testing primitive 'loadu' for neon", "[loadu],[neon]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_loadu_default<simd<double, neon>>());

        CHECK(test_loadu_default<simd<float, neon>>());

        CHECK(test_loadu_default<simd<int16_t, neon>>());

        CHECK(test_loadu_default<simd<int32_t, neon>>());

        CHECK(test_loadu_default<simd<int64_t, neon>>());

        CHECK(test_loadu_default<simd<int8_t, neon>>());

        CHECK(test_loadu_default<simd<uint16_t, neon>>());

        CHECK(test_loadu_default<simd<uint32_t, neon>>());

        CHECK(test_loadu_default<simd<uint64_t, neon>>());

        CHECK(test_loadu_default<simd<uint8_t, neon>>());
...
TEST_CASE("Testing primitive 'set' for neon", "[set],[neon]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive set not implemented for neon");
    }
}
...
TEST_CASE("Testing primitive 'set_zero' for neon", "[set_zero],[neon]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive set_zero not implemented for neon");
    }
}
...
TEST_CASE("Testing primitive 'extract_value' for neon", "[extract_value],[neon]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive extract_value not implemented for neon");
    }
}
...
TEST_CASE("Testing primitive 'gather' for neon", "[gather],[neon]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_gather_default<simd<double, neon>, simd<int64_t, neon>>());
        CHECK(test_gather_default<simd<double, neon>, simd<uint64_t, neon>>());

        CHECK(test_gather_default<simd<float, neon>, simd<int32_t, neon>>());
        CHECK(test_gather_default<simd<float, neon>, simd<uint32_t, neon>>());

        CHECK(test_gather_default<simd<int16_t, neon>, simd<int16_t, neon>>());
        CHECK(test_gather_default<simd<int16_t, neon>, simd<uint16_t, neon>>());

        CHECK(test_gather_default<simd<int32_t, neon>, simd<int32_t, neon>>());
        CHECK(test_gather_default<simd<int32_t, neon>, simd<uint32_t, neon>>());

        CHECK(test_gather_default<simd<int64_t, neon>, simd<int64_t, neon>>());
        CHECK(test_gather_default<simd<int64_t, neon>, simd<uint64_t, neon>>());

        CHECK(test_gather_default<simd<int8_t, neon>, simd<int8_t, neon>>());
        CHECK(test_gather_default<simd<int8_t, neon>, simd<uint8_t, neon>>());

        CHECK(test_gather_default<simd<uint16_t, neon>, simd<int16_t, neon>>());
        CHECK(test_gather_default<simd<uint16_t, neon>, simd<uint16_t, neon>>());

        CHECK(test_gather_default<simd<uint32_t, neon>, simd<int32_t, neon>>());
        CHECK(test_gather_default<simd<uint32_t, neon>, simd<uint32_t, neon>>());

        CHECK(test_gather_default<simd<uint64_t, neon>, simd<int64_t, neon>>());
        CHECK(test_gather_default<simd<uint64_t, neon>, simd<uint64_t, neon>>());

        CHECK(test_gather_default<simd<uint8_t, neon>, simd<int8_t, neon>>());
        CHECK(test_gather_default<simd<uint8_t, neon>, simd<uint8_t, neon>>());
...
TEST_CASE("Testing primitive 'masked_set1' for neon", "[masked_set1],[neon]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive masked_set1 not implemented for neon");
    }
}
...
TEST_CASE("Testing primitive 'sequence' for neon", "[sequence],[neon]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive sequence not implemented for neon");
    }
}
...
TEST_CASE("Testing primitive 'to_array' for neon", "[to_array],[neon]") {
    using namespace tsl;
    SECTION("default") {
        CHECK(test_to_array_default<simd<double, neon>>());

        CHECK(test_to_array_default<simd<float, neon>>());

        CHECK(test_to_array_default<simd<int16_t, neon>>());

        CHECK(test_to_array_default<simd<int32_t, neon>>());

        CHECK(test_to_array_default<simd<int64_t, neon>>());

        CHECK(test_to_array_default<simd<int8_t, neon>>());

        CHECK(test_to_array_default<simd<uint16_t, neon>>());

        CHECK(test_to_array_default<simd<uint32_t, neon>>());

        CHECK(test_to_array_default<simd<uint64_t, neon>>());

        CHECK(test_to_array_default<simd<uint8_t, neon>>());
...
TEST_CASE("Testing primitive 'custom_sequence' for neon", "[custom_sequence],[neon]") {
    using namespace tsl;
    SECTION("default") {
        WARN("Primitive custom_sequence not implemented for neon");
    }
}
